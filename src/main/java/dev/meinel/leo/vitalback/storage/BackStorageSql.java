...
public class BackStorageSql extends BackStorage {
    ...
    public Location loadBack(@NotNull Player player) {
        ...
        try (PreparedStatement selectStatement =
                SqlManager.getConnection().prepareStatement("SELECT * FROM ?Back")) {
    ...
    public void saveBack(@NotNull Player player) {
        ...
        try (PreparedStatement insertStatement = SqlManager.getConnection().prepareStatement(
                "INSERT INTO ?Back (`UUID`, `World`, `X`, `Y`, `Z`, `Yaw`, `Pitch`) VALUES (?, ?, ?, ?, ?, ?, ?)")) {
    ...
    protected void clear(@NotNull String playerUUID) {
        try (PreparedStatement deleteStatement =
                SqlManager.getConnection().prepareStatement("DELETE FROM ?Back WHERE `UUID`=?")) {
